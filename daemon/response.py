...
class Response():   
    ...
    def build_response_header(self, request):
        ...
        status_line = f"HTTP/1.1 {self.status_code if hasattr(self, 'status_code') and self.status_code else 200} OK\r\n"
        fmt_header = status_line
        
        # Merge basic headers
        for k, v in headers.items():
            if k not in rsphdr:  # rsphdr takes precedence if the app modified it
                fmt_header += f"{k}: {v}\r\n"
                
        # Merge extra/custom headers like Set-Cookie
    ...
    def build_response(self, request, envelop_content=None):
        ...
        # Prepare base_dir and envelop content based on MIME
        ...
        # Build header bytes natively if it has not been built
        if not self._header:
            self.build_response_header(request)

        return self._header + (self._content if isinstance(self._content, bytes) else b"")
